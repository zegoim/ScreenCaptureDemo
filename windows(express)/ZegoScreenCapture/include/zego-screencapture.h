...
/// \note 1. macOS下可以设置[window setSharingType:NSWindowSharingNone]过滤。
///       2. macOS下只有使用zego_screencapture_set_target_window过滤才生效。
///          macOS下使用zego_screencapture_set_target_screen过滤不生效。
...
/// \brief 设置采集分辨率
/// \param width 外部提供的分辨率宽
/// \param height 外部提供的分辨率高
/// \note width或height小于0时,使用原始采集目标分辨率
SCREENCAPTURE_API void zego_screencapture_set_resolution(int width, int height);
...
/// \brief 指定采集窗口列表
/// \param handle_list 待采集窗口列表
/// \param count 窗口数量
/// \param add true 添加采集窗口,false 移除采集窗口
SCREENCAPTURE_API int zego_screencapture_set_extra_target_windows(ZegoWindowHandle* handle_list, int count, bool add);
...
/// \brief 在采集目标为单个窗口的模式下,设置目标采集区域, 相对于窗口坐标
/// \param left 相对窗口左上角的横坐标
/// \param top 相对窗口左上角纵坐标
/// \param width 目标区域宽
/// \param height 目标区域高
/// \return 设置成功返回非0值,否则返回0
/// \note 在zego_screencapture_set_target_window后调用本接口进行设置。 
/// \note 1,如果不进行设置, 则默认截取整个窗口; 2,如果设置区域超过窗口区域, 则默认裁剪为窗口区域; 3,如果裁剪的区域为0,则采集整个窗口
/// \see zego_screencapture_set_target_window
...
/// \param notify 通知函数
/// \param data 用户自定义数据,回调时透传
SCREENCAPTURE_API void zego_screencapture_reg_mutiple_captured_frame_available_notify(zego_screencapture_mutiple_captured_frame_available_notify_func notify, void *data);
