...
class ZegoExpressEngineImp : public IZegoExpressEngine {
  public:
    ...
    void setPlayStreamVideoLayer(const std::string &streamID,
                                 ZegoPlayerVideoLayer videoLayer) override {
        const char *stream_id = streamID.c_str();
        oInternalOriginBridge->setPlayStreamVideoLayer(stream_id,
                                                       zego_player_video_layer(videoLayer));
    }
    ...
    void enableCameraAdaptiveFPS(bool enable, int minFPS, int maxFPS, ZegoPublishChannel channel) {
    ...
    void enableAudioDataCallback(bool enable, unsigned int callbackBitMask,
  ...
  public:
    ...
    static void setEngineConfig(ZegoEngineConfig engineConfig) {
        ...
        strncpy(_engineConfig.advanced_config, advanceConfig.c_str(), ZEGO_EXPRESS_MAX_COMMON_LEN);
